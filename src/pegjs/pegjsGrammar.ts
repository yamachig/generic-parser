...
//@ts-ignore
import ast from "pegjs-dev/packages/pegjs/lib/ast";

// eslint-disable-next-line @typescript-eslint/ban-ts-comment
//@ts-ignore
import util from "pegjs-dev/packages/pegjs/lib/util";

export const parse = (text: string): ast.Grammar => {
...
const Grammar = factory
    .sequence(s => s
        ...
        return new ast.Grammar( initializer as ast.Initializer | void, rules, comments as ast.CommentMap | void, location() );
    })
    .abstract();
...
const Initializer = factory
    .sequence(s => s
        ...
        return createNode( "initializer", { code }, location() ) as ast.Initializer;
    })
    .abstract();
...
const Rule = factory
    ...
    .action(({ name, displayName, createNode, expression, location }) => {
        const newExpression = displayName ? createNode( "named", {
            name: displayName,
            expression: expression,
        }, location() ) as ast.Named : expression;
        return createNode( "rule", { name, expression: newExpression }, location() ) as ast.Rule;
    })
    .abstract();
...
const ChoiceExpression = factory
    .sequence(s => s
        ...
        .action(({ head, tail, createNode, location }) => {
            ...
            return createNode( "choice", {
                alternatives: [ head ].concat( tail ),
            }, location() ) as ast.ChoiceExpression;
        })
    )
    .abstract();
...
const ActionExpression = factory
    .sequence(s => s
        .and(() => SequenceExpression, "expression")
        .and(r => r
            .zeroOrOne(r => r
                .sequence(s => s
                    .andOmit(() => __)
        ...
        if ( code === null ) return expression;
        return createNode( "action", { expression, code }, location() ) as ast.ActionExpression;
    })
    .abstract();
...
const SequenceExpression = factory
    .sequence(s => s
        .and(() => LabeledExpression, "head")
        .and(s => s
            .zeroOrMore(r => r
                .sequence(s => s
                    .andOmit(() => __)
                    .and(() => LabeledExpression)
                )
            )
        , "tail")
    ).action(({ head, tail, createNode, location }) => {
        ...
        return createNode( "sequence", { elements }, location() ) as ast.SequenceExpression;
    })
    .abstract();
...
const LabeledExpression = factory
    .choice(c => c
        .orSequence(s => s
            .and(r => r.seqEqual("@"))
            .and(r => r
                .zeroOrOne(() => LabelIdentifier),
            "label")
            .and(() => __)
            .and(() => PrefixedExpression, "expression")
            .action(({ pick, label, expression, createNode, location }) => {
                return createNode( "labeled", { pick, label, expression }, location() ) as ast.LabeledExpression;
            })
        )
        .orSequence(s => s
            ...
            .action(({ label, expression, createNode, location }) => {
                return createNode( "labeled", { label, expression }, location() ) as ast.LabeledExpression;
            })
        )
    ...
    .choice(c => c
        .orSequence(s => s
            ...
            .action(({ operator, expression, createNode, location }) => {
                return createNode( operator, { expression }, location() ) as ast.PrefixedExpression;
            })
        )
...
const PrefixedOperator = factory
    .choice(c => c
        ...
        .orSequence(s => s
            .and(r => r.seqEqual("&"))
            .action(() => "simple_and" as const)
        )
        .orSequence(s => s
            .and(r => r.seqEqual("!"))
            .action(() => "simple_not" as const)
        )
    )
    .abstract();
...
const SuffixedExpression = factory
    .choice(c => c
        .orSequence(s => s
            ...
            .action(({ operator, expression, createNode, location }) => {
                return createNode( operator, { expression }, location() ) as ast.SuffixedExpression;
            })
        )
...
const SuffixedOperator = factory
    .choice(c => c
        ...
        .orSequence(s => s
            .and(r => r.seqEqual("*"))
            .action(() => "zero_or_more" as const)
        )
        .orSequence(s => s
            .and(r => r.seqEqual("+"))
            .action(() => "one_or_more" as const)
        )
    )
    .abstract();
...
const PrimaryExpression = factory
    .choice(c => c
        .or(() => LiteralMatcher)
        .or(() => CharacterClassMatcher)
        .or(() => AnyMatcher)
        .or(() => RuleReferenceExpression)
        .or(() => SemanticPredicateExpression)
        .orSequence(s => s
            .and(r => r.seqEqual("("))
            .and(() => __)
            .and(() => Expression, "e")
            ...
            .action(({ e, createNode, location }) => {
                if ( e.type !== "labeled" && e.type !== "sequence" ) return e;
                return createNode( "group", { expression: e }, location() ) as ast.GroupExpression;
            })
        )
    )
    .abstract();
...
const RuleReferenceExpression = factory
    .sequence(s => s
        ...
        .action(({ name, createNode, location }) => {
            return createNode( "rule_ref", { name }, location() ) as ast.RuleReferenceExpression;
        })
    )
    .abstract();
...
const SemanticPredicateExpression = factory
    .sequence(s => s
        ...
        .action(({ operator, code, createNode, location }) => {
            return createNode( operator, { code }, location() ) as ast.SemanticPredicateExpression;
        })
    )
    .abstract();
...
const SemanticPredicateOperator = factory
    .choice(c => c
        .orSequence(s => s
            .and(r => r.seqEqual("&"))
            .action(() => "semantic_and" as const)
        )
        .orSequence(s => s
            .and(r => r.seqEqual("!"))
            .action(() => "semantic_not" as const)
        )
    );
...
const SingleLineComment = factory
    .sequence(s => s
        ...
        .and(r => r.asSlice(r => r
            .zeroOrMore(r => r
                .sequence(s => s
                    .and(r => r.nextIsNot(() => LineTerminator))
                    .and(() => SourceCharacter)
                )
            )
        ), "comment")
...
const IdentifierPart = factory
    .choice(c => c
        .or(() => IdentifierStart)
        .or(() => UnicodeCombiningMark)
        .or(() => UnicodeDigit)
        .or(() => UnicodeConnectorPunctuation)
        .or(r => r.seqEqual("\u200C"))
        .or(r => r.seqEqual("\u200D"))
    )
    .abstract();
...
const LiteralMatcher = factory
    ...
    .action(({ value, ignoreCase, createNode, location }) => {
        return createNode( "literal", {
            value: value,
            ignoreCase: ignoreCase !== null,
        }, location() ) as ast.LiteralMatcher;
    })
    .abstract();
...
const CharacterClassMatcher = factory
    ...
    .sequence(s => s
        .and(r => r.seqEqual("["))
        .and(r => r.zeroOrOne(r => r.seqEqual("^")), "inverted")
        ...
        return createNode( "class", {
            ...
            inverted: inverted !== null,
            ignoreCase: ignoreCase !== null,
        }, location() ) as ast.CharacterClassMatcher;
    })
    .abstract();
...
const EscapeSequence = factory
    .choice(c => c
        .or(() => CharacterEscapeSequence)
        .orSequence(s => s
            .and(r => r.seqEqual("0"))
            .and(r => r.nextIsNot(() => DecimalDigit))
            .action(() => "\0")
        )
        .or(() => HexEscapeSequence)
...
const CharacterEscapeSequence = factory
    .choice(c => c
        .or(() => SingleEscapeCharacter)
        .or(() => NonEscapeCharacter)
    )
    .abstract();
...
const AnyMatcher = factory
    .sequence(s => s
        ...
        .action(({ createNode, location }) => {
            return createNode( "any", undefined, location() ) as ast.AnyMatcher;
        })
    )
    .abstract();
...
const _ = factory
    .zeroOrMore(r => r
        .choice(c => c
            .or(() => WhiteSpace)
            .or(() => MultiLineCommentNoLineTerminator)
        )
    )
    .abstract();
