...
const initializer = (options: peg.parser.IOptions) => {
    ...
    const reservedWords = options.reservedWords || pegjs.util.reservedWords;
    if ( Array.isArray( reservedWords ) ) reservedWords.forEach( word => {
        RESERVED_WORDS[ word ] = true;
    } );
...
const Expression: Rule<string, peg.ast.Expression, Env, Empty> = factory
...
const Code: Rule<string, string, Env, Empty> = factory
    .asSlice(r => r
        .zeroOrMore(r => r
            .choice(c => c
